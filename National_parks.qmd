...
all_state_choices <- c(state.name, "District of Columbia", "Puerto Rico", "U.S. Virgin Islands", "American Samoa", "Guam", "Northern Mariana Islands")
...
build_state_polygons <- function() {
  ...
  territory_centers <- tibble(
    state = c("District of Columbia", "Puerto Rico", "U.S. Virgin Islands", "American Samoa", "Guam", "Northern Mariana Islands"),
    lat = c(38.9072, 18.2208, 18.3358, -14.2710, 13.4443, 15.0979),
    lon = c(-77.0369, -66.5901, -64.8963, -170.1322, 144.7937, 145.6739)
  )
  
  list(polygons = state_map, centers = bind_rows(state_centers, territory_centers), territories = territory_centers)
}
```
...
server <- function(input, output, session) {
  ...
  observe({
    updateSelectInput(session, "states", 
                     choices = c("All States/Territories" = "ALL", all_state_choices),
                     selected = "ALL")
  })
  
  observeEvent(input$states, {
    if (is.null(input$states) || "ALL" %in% input$states) {
  ...
  output$state_selector_map <- renderLeaflet({
    ...
    leaflet(polygon_data) %>%
      ...
      addCircleMarkers(
        data = state_map_data$territories,
        lng = ~lon,
        lat = ~lat,
        layerId = ~state,
        radius = 6,
        color = "#555555",
        fillColor = ~if_else(state %in% selected, "#1f78b4", "#d9d9d9"),
        fillOpacity = 0.8,
        label = ~state
      ) %>%
  ...
  observe({
    ...
    max_limit <- if ("ALL" %in% input$states || length(input$states) == 0) 51 else max_parks
  ...
  output$status_text <- renderText({
    ...
    selected_count <- length(input$selected_parks)

    if (!is.null(start_loc)) {
      sprintf("Ready to calculate route for %d parks (%d explicitly selected) starting from %s, %s", 
              nrow(parks), selected_count, start_loc$city, start_loc$state)
    } else {
      sprintf("Ready to calculate route for %d parks (%d explicitly selected)", nrow(parks), selected_count)
    }
  })
